    ...
    return (
        <>
                    ...
                    <div className="flex text-lg space-x-4 justify-center md:justify-start">
                        {socialMediaLinks.map((link, index) => (
                            <a key={index} href={link.href}>
                                <div className="flex items-center justify-center"><i className={link.iconClass}></i></div>
                            </a>
                        ))}
                    </div>
                </div>
            ...
            <div className="bg-gray-200 p-2 text-xl text-green-900 font-bold text-center">
                <h2> Ghar <span className="text-yellow-800 line-through decoration-green-900">Jaisa</span> Ka Aachar! <br className='sm:hidden'/>बदलेंगे स्वाद </h2>
            </div>
                ...
                <div className="absolute bg-gray-800 text-white mt-2 rounded shadow-lg">
                    <ul className="flex flex-col">
                        {headerLinks.map((heading, index) => (
                            <li key={index}>
                                <a href={heading.href} className="block py-2 px-4 hover:bg-gray-700">
                                    {heading.name}
                                </a>
                            </li>
                        ))}
